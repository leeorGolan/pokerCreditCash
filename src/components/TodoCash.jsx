...
function Todo({task,toggleComplete,deleteTodo,editTodo,num,plus,minus,taskCash}) {
      ...
      {/* <Box display={'flex'} flexDirection={'row'} sx={{backgroundColor:'red'}}> */}
        ...
        <IconButton
          ...
          >
            ...
            <IconButton sx={{color:'#012030'}} variant='contained' onClick= {() => minus(taskCash.id)}><RemoveIcon/></IconButton>

            <Typography color= {taskCash.num < 0 ? red[800]:'green'}  variant='h4'>{taskCash.num}</Typography>

            <IconButton sx={{color:'#012030'}} variant='contained' onClick= {() => plus(taskCash.id)}><AddIcon/></IconButton>
           ...
           <Box display={'flex'} flexDirection={'row'} alignItems={'center'} justifyContent={'flex-end'}  minWidth={150} sx={{backgroundColor:''}}>
            ...
            onClick={() => toggleComplete(taskCash.id)}>{taskCash.task}</Typography>
        </Box>
      {/* </Box> */}
